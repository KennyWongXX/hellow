...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$bat='%~f0'; $lines=[System.IO.File]::ReadAllLines($bat); $s=-1; $e=$lines.Length; for($i=0;$i -lt $lines.Length;$i++){ $t=$lines[$i].Trim(); if($t -eq '<<SCRIPT>>'){ $s=$i+1 }; if($t -eq '<<END>>'){ $e=$i; break } }; if($s -lt 0){ Write-Host 'ERROR: Script block not found in bat file' -ForegroundColor Red; exit 1 }; $script=($lines[$s..($e-1)] -join [Environment]::NewLine); [System.IO.File]::WriteAllText('%RUNPS1%',$script,[System.Text.UTF8Encoding]::new($false))"
if errorlevel 1 goto :failed

powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%RUNPS1%"
...
if "%SCAN_EXIT%"=="0" (
  ...
  echo   Scan finished with warnings/errors - code %SCAN_EXIT%
)
echo   Report saved in: %TEMP%
...
:failed
echo.
echo  ERROR: Could not prepare scan script.
